//! HTML Plugin
//!
//! Provides HTML language support via vscode-html-language-server.
...
/// Type alias for HTML LSP client
pub type HtmlLspClient = LspClient<HtmlConfig>;

/// HTML plugin state
pub struct HtmlPlugin {
    manifest: PluginManifest,
    ctx: Option<PluginContext>,
    lsp: Arc<RwLock<Option<HtmlLspClient>>>,
...
impl HtmlPlugin {
    pub fn new() -> Self {
        Self {
            manifest: PluginManifest {
                id: "panager.html".to_string(),
                name: "HTML".to_string(),
                version: "1.0.0".to_string(),
                description: "HTML language support".to_string(),
    ...
    async fn stop_lsp(&mut self) {
        ...
        if let Some(ref ctx) = self.ctx {
            ctx.remove_status_bar("html-status".to_string());
        }
    }
}
...
impl Plugin for HtmlPlugin {
    ...
    async fn activate(&mut self, ctx: PluginContext) -> Result<(), String> {
        info!("Activating HTML plugin");
        self.ctx = Some(ctx);
        Ok(())
    }

    async fn deactivate(&mut self) -> Result<(), String> {
        info!("Deactivating HTML plugin");
        self.stop_lsp().await;
        self.ctx = None;
        Ok(())
    }
        ...
        match event {
            HostEvent::ProjectOpened { path } => {
                ...
                tokio::spawn(async move {
                    info!("Starting HTML LSP for: {:?}", path);
                    match LspClient::start(&config, &path, ctx.clone().unwrap()).await {
                        Ok(client) => {
                            *lsp.write().await = Some(client);
                            if let Some(ctx) = ctx {
                                ctx.update_status_bar(StatusBarItem {
                                    id: "html-status".to_string(),
                                    text: "HTML".to_string(),
                                    tooltip: Some("HTML Language Server running".to_string()),
                                    alignment: StatusBarAlignment::Right,
                                    priority: 47,
                                });
                            }
                        }
                        Err(e) => warn!("Failed to start HTML LSP: {}", e),
                    }
                });
            }
            ...
            HostEvent::FileOpened { path, content, language } => {
                debug!("HTML plugin received FileOpened: {:?}, lang: {}", path, language);
...
#[async_trait]
impl LspProvider for HtmlPlugin {

...
export function IdeSettingsDialog({ open, onOpenChange }: IdeSettingsDialogProps) {
  ...
  const { loadSettings, loadSettingsForLevel, loadAllLevelSettings, scopeDefaultFolder } =
    useIdeSettingsStore();

  // Currently selected settings level for editing
  const [selectedLevel, setSelectedLevel] = useState<SettingsLevel>("workspace");
  ...
  }, [open, projectContext, loadSettings, loadSettingsForLevel, loadAllLevelSettings, selectedLevel]);
  ...
  }, [selectedLevel, open, projectContext, loadSettingsForLevel]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] p-0">
        ...
        <Tabs.Root defaultValue="general" className="flex sm:max-w-[700px] max-h-[500px]">
          <Tabs.List
            className={cn(
          ...
          <div className="flex-1 min-w-0 flex flex-col">
            <div className="flex-1 overflow-y-auto">
              ...
              <Tabs.Content value="general" className="px-6 pt-2 pb-6 outline-none">
                <GeneralSettingsTab level={selectedLevel} />
              </Tabs.Content>
              <Tabs.Content value="editor" className="px-6 pt-2 pb-6 outline-none">
...
function LevelSelector({
  ...
  const levels: { level: SettingsLevel; label: string; icon: React.ReactNode; disabled?: boolean }[] = [
    { level: "user", label: "User", icon: <User className="w-4 h-4" /> },
    { level: "scope", label: "Scope", icon: <Building2 className="w-4 h-4" />, disabled: !canEditScope },
    { level: "workspace", label: projectName || "Workspace", icon: <FolderOpen className="w-4 h-4" /> },
  ];

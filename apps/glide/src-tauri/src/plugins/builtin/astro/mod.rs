//! Astro Plugin
//!
//! Provides Astro language support via @astrojs/language-server.
//! Auto-activates when astro dependency or .astro files are detected.
...
impl AstroConfig {
    /// Check if a project uses Astro
    pub fn has_astro(root: &PathBuf) -> bool {
...
/// Type alias for Astro LSP client
pub type AstroLspClient = LspClient<AstroConfig>;

/// Astro plugin state
pub struct AstroPlugin {
    manifest: PluginManifest,
    ctx: Option<PluginContext>,
    lsp: Arc<RwLock<Option<AstroLspClient>>>,
...
impl AstroPlugin {
    pub fn new() -> Self {
        Self {
            manifest: PluginManifest {
                id: "panager.astro".to_string(),
                name: "Astro".to_string(),
                version: "1.0.0".to_string(),
                description: "Astro language support".to_string(),
                languages: vec!["astro".to_string()],
                is_builtin: true,
            },
            ctx: None,
    ...
    async fn stop_lsp(&mut self) {
        ...
        if let Some(ref ctx) = self.ctx {
            ctx.remove_status_bar("astro-status".to_string());
        }
    }
}

impl Default for AstroPlugin {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Plugin for AstroPlugin {
    ...
    async fn activate(&mut self, ctx: PluginContext) -> Result<(), String> {
        info!("Activating Astro plugin");
        self.ctx = Some(ctx);
        Ok(())
    }

    async fn deactivate(&mut self) -> Result<(), String> {
        info!("Deactivating Astro plugin");
        self.stop_lsp().await;
        self.ctx = None;
        Ok(())
    }
        ...
        match event {
            HostEvent::ProjectOpened { path } => {
                ...
                tokio::spawn(async move {
                    info!("Starting Astro LSP for: {:?}", path);
                    match LspClient::start(&config, &path, ctx.clone().unwrap()).await {
                        Ok(client) => {
                            *lsp.write().await = Some(client);
                            if let Some(ctx) = ctx {
                                ctx.update_status_bar(StatusBarItem {
                                    id: "astro-status".to_string(),
                                    text: "Astro".to_string(),
                                    tooltip: Some("Astro language server active".to_string()),
                                    alignment: StatusBarAlignment::Right,
                                    priority: 50,
                                });
                            }
                        }
                        Err(e) => warn!("Failed to start Astro LSP: {}", e),
                    }
                });
            }
            ...
            HostEvent::FileOpened { path, content, language } => {
                debug!("Astro plugin received FileOpened: {:?}, lang: {}", path, language);
...
#[async_trait]
impl LspProvider for AstroPlugin {

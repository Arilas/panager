//! Svelte Plugin
//!
//! Provides Svelte language support via svelte-language-server.
//! Auto-activates when svelte dependency or svelte.config is detected.
...
impl SvelteConfig {
    /// Check if a project uses Svelte
    pub fn has_svelte(root: &PathBuf) -> bool {
        // Check for svelte.config file
        let config_files = ["svelte.config.js", "svelte.config.ts", "svelte.config.mjs"];
...
impl LspConfig for SvelteConfig {
    ...
    fn args(&self) -> Vec<String> {
        vec![
            "--yes".to_string(),
            "svelte-language-server".to_string(),
            "--stdio".to_string(),
        ]
    }
    ...
    fn should_activate(&self, root: &PathBuf) -> bool {
        Self::has_svelte(root)
    }
}

/// Type alias for Svelte LSP client
pub type SvelteLspClient = LspClient<SvelteConfig>;

/// Svelte plugin state
pub struct SveltePlugin {
    manifest: PluginManifest,
    ctx: Option<PluginContext>,
    lsp: Arc<RwLock<Option<SvelteLspClient>>>,
...
impl SveltePlugin {
    pub fn new() -> Self {
        Self {
            manifest: PluginManifest {
                id: "panager.svelte".to_string(),
                name: "Svelte".to_string(),
                version: "1.0.0".to_string(),
                description: "Svelte language support".to_string(),
                languages: vec!["svelte".to_string()],
                is_builtin: true,
            },
            ctx: None,
    ...
    async fn stop_lsp(&mut self) {
        ...
        if let Some(ref ctx) = self.ctx {
            ctx.remove_status_bar("svelte-status".to_string());
        }
    }
}
...
impl Plugin for SveltePlugin {
    ...
    async fn activate(&mut self, ctx: PluginContext) -> Result<(), String> {
        info!("Activating Svelte plugin");
        self.ctx = Some(ctx);
        Ok(())
    }

    async fn deactivate(&mut self) -> Result<(), String> {
        info!("Deactivating Svelte plugin");
        self.stop_lsp().await;
        self.ctx = None;
        Ok(())
    }
        ...
        match event {
            HostEvent::ProjectOpened { path, .. } => {
                ...
                tokio::spawn(async move {
                    info!("Starting Svelte LSP for: {:?}", path);
                    match LspClient::start(&config, &path, ctx.clone().unwrap()).await {
                        Ok(client) => {
                            *lsp.write().await = Some(client);
                            if let Some(ctx) = ctx {
                                ctx.update_status_bar(StatusBarItem {
                                    id: "svelte-status".to_string(),
                                    text: "Svelte".to_string(),
                                    tooltip: Some("Svelte language server active".to_string()),
                                    alignment: StatusBarAlignment::Right,
                                    priority: 51,
                                });
                            }
                        }
                        Err(e) => warn!("Failed to start Svelte LSP: {}", e),
                    }
                });
            }
            ...
            HostEvent::FileOpened { path, content, language } => {
                debug!("Svelte plugin received FileOpened: {:?}, lang: {}", path, language);
...
#[async_trait]
impl LspProvider for SveltePlugin {

//! Filesystem utilities for Glide
//!
...
pub fn create_dir_all(path: &str) -> Result<()> {
    let path = super::paths::expand_tilde(path);
    fs::create_dir_all(&path).map_err(GlideError::Io)
}
...
pub fn remove_dir_all(path: &str) -> Result<()> {
    let path = super::paths::expand_tilde(path);
    fs::remove_dir_all(&path).map_err(GlideError::Io)
}
...
pub fn read_to_string(path: &str) -> Result<String> {
    let path = super::paths::expand_tilde(path);
    fs::read_to_string(&path).map_err(GlideError::Io)
}
...
pub fn write_string(path: &str, contents: &str) -> Result<()> {
    ...
    if let Some(parent) = Path::new(&path).parent() {
        if !parent.exists() {
            fs::create_dir_all(parent).map_err(GlideError::Io)?;
        }
    }

    fs::write(&path, contents).map_err(GlideError::Io)
}
...
pub fn append_string(path: &str, contents: &str) -> Result<()> {
    ...
    let mut file = OpenOptions::new()
        ...
        .append(true)
        .open(&path)
        .map_err(GlideError::Io)?;

    file.write_all(contents.as_bytes()).map_err(GlideError::Io)
}
...
pub fn move_path(from: &str, to: &str) -> Result<()> {
    ...
    fs::rename(&from, &to).map_err(GlideError::Io)
}
...
pub fn copy_file(from: &str, to: &str) -> Result<u64> {
    ...
    fs::copy(&from, &to).map_err(GlideError::Io)
}

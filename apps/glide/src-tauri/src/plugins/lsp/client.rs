//! Generic LSP Client
//!
//! This module provides a reusable LSP client that can be configured
//! for different language servers via the LspConfig trait.
...
pub struct LspClient<C: LspConfig> {
    ...
    /// Configuration (phantom data since we only need methods)
...
impl<C: LspConfig> LspClient<C> {
    ...
    pub async fn start(config: &C, root: &PathBuf, ctx: PluginContext) -> Result<Self, String> {
        ...
        // Get workspace config for message handling
        let workspace_config = config.workspace_configuration();
        let diagnostic_source = config.diagnostic_source().to_string();
    ...
    async fn initialize(&self, config: &C) {
        ...
        let params = serde_json::json!({
            ...
            "initializationOptions": config.initialization_options(&self.root)
        });
        ...
        let workspace_config = config.workspace_configuration();
        if !workspace_config.is_null() && workspace_config != serde_json::json!({}) {
    ...
    async fn handle_message(
        ...
        if msg.get("method") == Some(&serde_json::json!("textDocument/publishDiagnostics")) {
            if let Some(params) = msg.get("params") {
                Self::handle_diagnostics(ctx, params, diagnostic_source).await;
            }
        }
    }

    /// Handle publishDiagnostics notification
    async fn handle_diagnostics(ctx: &PluginContext, params: &serde_json::Value, source: &str) {
        ...
        for (idx, diag) in diagnostics_json.iter().enumerate() {
            ...
            diagnostics.push(Diagnostic {
                id: format!("{}-{}-{}", source.to_lowercase(), file_path, idx),
        ...
        debug!("Publishing {} diagnostics for {} (source: {})", diagnostics.len(), file_path, source);
    ...
    fn parse_hover(value: serde_json::Value) -> Result<Option<LspHover>, String> {
        ...
        Ok(Some(LspHover { contents: markup, range }))
    }
    ...
    fn parse_markup_content(value: &serde_json::Value) -> Result<LspMarkupContent, String> {
        ...
        if value.is_object() {
            ...
            if let Some(val) = value.get("value").and_then(|v| v.as_str()) {
                let lang = value.get("language").and_then(|l| l.as_str()).unwrap_or("text");
                return Ok(LspMarkupContent {
            ...
            let parts: Vec<String> = arr
                ...
                .filter_map(|item| {
                    if let Some(s) = item.as_str() {
                        ...
                        let lang = item.get("language").and_then(|l| l.as_str()).unwrap_or("text");
                        Some(format!("```{}\n{}\n```", lang, val))
                    } else {
                        None
                    }
    ...
    fn parse_completion_list(value: serde_json::Value) -> Result<LspCompletionList, String> {
        let (is_incomplete, items) = if let Some(arr) = value.as_array() {
            ...
            let is_incomplete = value.get("isIncomplete").and_then(|v| v.as_bool()).unwrap_or(false);
            let items = value.get("items").and_then(|v| v.as_array()).cloned().unwrap_or_default();
            (is_incomplete, items)
        };

        let items: Vec<LspCompletionItem> = items.iter().filter_map(Self::parse_completion_item).collect();

        Ok(LspCompletionList { is_incomplete, items })
    }
    ...
    fn parse_completion_item(value: &serde_json::Value) -> Option<LspCompletionItem> {
        ...
        let insert_text = value.get("insertText").and_then(|t| t.as_str()).map(|s| s.to_string());
        let insert_text_format = value.get("insertTextFormat").and_then(|f| f.as_u64()).map(|f| f as u32);
        let sort_text = value.get("sortText").and_then(|t| t.as_str()).map(|s| s.to_string());
        let filter_text = value.get("filterText").and_then(|t| t.as_str()).map(|s| s.to_string());
    ...
    fn parse_workspace_edit(value: serde_json::Value) -> Result<LspWorkspaceEdit, String> {
            ...
            if let Some(obj) = c.as_object() {
                ...
                for (uri, edits) in obj {
                    if let Some(arr) = edits.as_array() {
                        let text_edits: Vec<LspTextEdit> = arr.iter().filter_map(Self::parse_text_edit).collect();
        ...
        Ok(LspWorkspaceEdit { changes, document_changes: None })
    }
    ...
    fn parse_code_actions(value: serde_json::Value) -> Result<Vec<LspCodeAction>, String> {
        ...
        let actions: Vec<LspCodeAction> = arr
            ...
            .filter_map(|item| {
                ...
                let edit = item.get("edit").and_then(|e| Self::parse_workspace_edit(e.clone()).ok());
    ...
    fn parse_document_symbols(value: serde_json::Value) -> Result<Vec<LspDocumentSymbol>, String> {
        ...
        let symbols: Vec<LspDocumentSymbol> = arr.iter().filter_map(Self::parse_document_symbol).collect();
        Ok(symbols)
    }
    ...
    fn parse_document_symbol(value: &serde_json::Value) -> Option<LspDocumentSymbol> {
        ...
        let children = value.get("children").and_then(|c| {
            c.as_array().map(|arr| arr.iter().filter_map(Self::parse_document_symbol).collect())
        });
    ...
    fn parse_inlay_hints(value: serde_json::Value) -> Result<Vec<LspInlayHint>, String> {
        ...
        let hints: Vec<LspInlayHint> = arr.iter().filter_map(Self::parse_inlay_hint).collect();
        Ok(hints)
    }
    ...
    fn parse_document_highlights(value: serde_json::Value) -> Result<Vec<LspDocumentHighlight>, String> {
        if value.is_null() {
            return Ok(vec![]);
        }
    ...
    fn parse_signature_help(value: serde_json::Value) -> Result<Option<LspSignatureHelp>, String> {
        if value.is_null() {
            return Ok(None);
        }

        let signatures_arr = value.get("signatures").and_then(|s| s.as_array()).ok_or("Expected signatures array")?;

        let signatures: Vec<LspSignatureInformation> = signatures_arr
            .iter()
            .filter_map(|sig| {
                let label = sig.get("label")?.as_str()?.to_string();
                let documentation = sig.get("documentation").and_then(|d| Self::parse_markup_content(d).ok());
                let parameters = sig.get("parameters").and_then(|p| p.as_array()).map(|arr| {
                    arr.iter()
                        .filter_map(|param| {
                            let label = Self::parse_parameter_label(param.get("label")?)?;
                            let documentation = param.get("documentation").and_then(|d| Self::parse_markup_content(d).ok());
                            Some(LspParameterInformation { label, documentation })
                        })
                        .collect()
                });
                let active_parameter = sig.get("activeParameter").and_then(|a| a.as_u64()).map(|a| a as u32);
        ...
        let active_signature = value.get("activeSignature").and_then(|a| a.as_u64()).map(|a| a as u32);
        let active_parameter = value.get("activeParameter").and_then(|a| a.as_u64()).map(|a| a as u32);
    ...
    fn parse_text_edits(value: serde_json::Value) -> Result<Vec<LspTextEdit>, String> {
        ...
        let edits: Vec<LspTextEdit> = arr.iter().filter_map(Self::parse_text_edit).collect();
        Ok(edits)
    }
    ...
    fn parse_folding_ranges(value: serde_json::Value) -> Result<Vec<LspFoldingRange>, String> {
        ...
        let ranges: Vec<LspFoldingRange> = arr
            ...
            .filter_map(|item| {
                ...
                let start_character = item.get("startCharacter").and_then(|c| c.as_u64()).map(|c| c as u32);
                let end_character = item.get("endCharacter").and_then(|c| c.as_u64()).map(|c| c as u32);
                let kind = item.get("kind").and_then(|k| k.as_str()).map(|s| s.to_string());
    ...
    fn parse_selection_ranges(value: serde_json::Value) -> Result<Vec<LspSelectionRange>, String> {
        ...
        let ranges: Vec<LspSelectionRange> = arr.iter().filter_map(Self::parse_selection_range).collect();
        Ok(ranges)
    }

    fn parse_selection_range(value: &serde_json::Value) -> Option<LspSelectionRange> {
        let range = Self::parse_range(value.get("range")?)?;
        let parent = value.get("parent").and_then(Self::parse_selection_range).map(Box::new);
        Some(LspSelectionRange { range, parent })
    }

    fn parse_linked_editing_ranges(value: serde_json::Value) -> Result<Option<LspLinkedEditingRanges>, String> {
        if value.is_null() {
            return Ok(None);
        }

        let ranges_arr = value.get("ranges").and_then(|r| r.as_array()).ok_or("Expected ranges array")?;
        let ranges: Vec<LspRange> = ranges_arr.iter().filter_map(Self::parse_range).collect();
        let word_pattern = value.get("wordPattern").and_then(|w| w.as_str()).map(|s| s.to_string());

        Ok(Some(LspLinkedEditingRanges { ranges, word_pattern }))
    }
    ...
    pub async fn did_open(&self, config: &C, path: &PathBuf, content: &str) {
        let uri = format!("file://{}", path.display());
        let language_id = config.language_id(
            path.extension().and_then(|e| e.to_str()).unwrap_or("")
        );

        debug!("LSP didOpen: uri={}, languageId={}", uri, language_id);

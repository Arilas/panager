//! CSS Plugin
//!
//! Provides CSS, SCSS, and Less language support via vscode-css-language-server.
...
/// Type alias for CSS LSP client
pub type CssLspClient = LspClient<CssConfig>;

/// CSS plugin state
pub struct CssPlugin {
    manifest: PluginManifest,
    ctx: Option<PluginContext>,
    lsp: Arc<RwLock<Option<CssLspClient>>>,
...
impl CssPlugin {
    pub fn new() -> Self {
        Self {
            manifest: PluginManifest {
                id: "panager.css".to_string(),
                name: "CSS".to_string(),
                version: "1.0.0".to_string(),
                description: "CSS, SCSS, and Less language support".to_string(),
                languages: vec!["css".to_string(), "scss".to_string(), "less".to_string()],
                is_builtin: true,
            },
            ctx: None,
    ...
    async fn stop_lsp(&mut self) {
        ...
        if let Some(ref ctx) = self.ctx {
            ctx.remove_status_bar("css-status".to_string());
        }
    }
}
...
impl Plugin for CssPlugin {
    ...
    async fn activate(&mut self, ctx: PluginContext) -> Result<(), String> {
        info!("Activating CSS plugin");
        self.ctx = Some(ctx);
        Ok(())
    }

    async fn deactivate(&mut self) -> Result<(), String> {
        info!("Deactivating CSS plugin");
        self.stop_lsp().await;
        self.ctx = None;
        Ok(())
    }
        ...
        match event {
            HostEvent::ProjectOpened { path, .. } => {
                ...
                tokio::spawn(async move {
                    info!("Starting CSS LSP for: {:?}", path);
                    match LspClient::start(&config, &path, ctx.clone().unwrap()).await {
                        Ok(client) => {
                            *lsp.write().await = Some(client);
                            if let Some(ctx) = ctx {
                                ctx.update_status_bar(StatusBarItem {
                                    id: "css-status".to_string(),
                                    text: "CSS".to_string(),
                                    tooltip: Some("CSS Language Server running".to_string()),
                                    alignment: StatusBarAlignment::Right,
                                    priority: 48,
                                });
                            }
                        }
                        Err(e) => warn!("Failed to start CSS LSP: {}", e),
                    }
                });
            }
            ...
            HostEvent::FileOpened { path, content, language } => {
                debug!("CSS plugin received FileOpened: {:?}, lang: {}", path, language);
...
#[async_trait]
impl LspProvider for CssPlugin {

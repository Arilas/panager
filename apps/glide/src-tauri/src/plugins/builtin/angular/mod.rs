//! Angular Plugin
//!
//! Provides Angular language support via @angular/language-server.
//! Auto-activates when Angular dependency or angular.json is detected.
...
impl AngularConfig {
    /// Check if a project uses Angular
    pub fn has_angular(root: &PathBuf) -> bool {
        // Check for angular.json (Angular CLI config)
        if root.join("angular.json").exists() {
            return true;
        }

        // Check package.json for @angular/core dependency
...
impl LspConfig for AngularConfig {
    ...
    fn args(&self) -> Vec<String> {
        vec![
            "--yes".to_string(),
            "@angular/language-server".to_string(),
            "--stdio".to_string(),
    ...
    fn workspace_configuration(&self) -> serde_json::Value {
        serde_json::json!({
            "angular": {
                "enable-experimental-ivy-prompt": false
            },
            "typescript": {
                "preferences": {
    ...
    fn should_activate(&self, root: &PathBuf) -> bool {
        Self::has_angular(root)
    }
}

/// Type alias for Angular LSP client
pub type AngularLspClient = LspClient<AngularConfig>;

/// Angular plugin state
pub struct AngularPlugin {
    manifest: PluginManifest,
    ctx: Option<PluginContext>,
    lsp: Arc<RwLock<Option<AngularLspClient>>>,
...
impl AngularPlugin {
    pub fn new() -> Self {
        Self {
            manifest: PluginManifest {
                id: "panager.angular".to_string(),
                name: "Angular".to_string(),
                version: "1.0.0".to_string(),
                description: "Angular language support".to_string(),
                languages: vec![
    ...
    async fn stop_lsp(&mut self) {
        ...
        if let Some(ref ctx) = self.ctx {
            ctx.remove_status_bar("angular-status".to_string());
        }
    }
}

impl Default for AngularPlugin {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Plugin for AngularPlugin {
    ...
    async fn activate(&mut self, ctx: PluginContext) -> Result<(), String> {
        info!("Activating Angular plugin");
        self.ctx = Some(ctx);
        Ok(())
    }

    async fn deactivate(&mut self) -> Result<(), String> {
        info!("Deactivating Angular plugin");
        self.stop_lsp().await;
        self.ctx = None;
        Ok(())
    }
        ...
        match event {
            HostEvent::ProjectOpened { path, .. } => {
                ...
                tokio::spawn(async move {
                    info!("Starting Angular LSP for: {:?}", path);
                    match LspClient::start(&config, &path, ctx.clone().unwrap()).await {
                        Ok(client) => {
                            *lsp.write().await = Some(client);
                            if let Some(ctx) = ctx {
                                ctx.update_status_bar(StatusBarItem {
                                    id: "angular-status".to_string(),
                                    text: "Angular".to_string(),
                                    tooltip: Some("Angular language server active".to_string()),
                                    alignment: StatusBarAlignment::Right,
                                    priority: 52,
                                });
                            }
                        }
                        Err(e) => warn!("Failed to start Angular LSP: {}", e),
                    }
                });
            }
            ...
            HostEvent::FileOpened { path, content, language } => {
                debug!("Angular plugin received FileOpened: {:?}, lang: {}", path, language);
...
#[async_trait]
impl LspProvider for AngularPlugin {

...
export function FileTreePanel() {
  ...
  const handleOpenInTerminal = async () => {
    ...
    try {
      // Use Tauri shell plugin to open terminal
      // The 'open' function with the directory will open Finder on macOS,
      // so we use a terminal-specific approach
      const isMac = navigator.platform.toLowerCase().includes("mac");
      if (isMac) {
        // Try to open Terminal.app with the directory
        ...
        // On other platforms, open the directory which may trigger default file manager
        await open(dirPath);
      }
    } catch (err) {
